...
for q in sample_prompts(batch):
    ...
    for o_i, A_i in zip(group, A):
        ratio = policy.logprob(o_i) / old_policy.logprob(o_i)     # per token
...
**Off-policy distillation (SFT on a teacher's traces).** DeepSeek-R1 reports fine-tuning dense Qwen and Llama models on roughly 800K reasoning traces generated by R1, and that the distilled 32B model outperforms a much larger RL-on-the-small-model alternative. The lesson: for a small model on a known domain, distilling from a strong reasoner is more effective and far cheaper than doing your own RL.
